...
export default function Login() {
  ...
  // Only attempt GIS in real browsers
  useEffect(() => {
    if (inApp) return;
    let tries = 0;
    const max = 40;
  ...
  // Render Google button (GIS → ID token)
  useEffect(() => {
    if (inApp) return;
    ...
    const mount = document.getElementById("googleLoginBtnMount");
    if (mount) {
  ...
  const copyLink = async () => {
    const url = "https://www.starsignstudio.com/login";
    try {
  ...
  const WarningCard = () => (
    <div className="rounded-md border border-yellow-500/40 bg-yellow-500/10 px-4 py-3 text-sm text-yellow-100 space-y-3 text-center">
      <div>
        You’re inside an in-app browser (<b>{inApp}</b>). Google login is blocked or unreliable here.
      ...
      <div className="w-full max-w-sm space-y-5">
        ...
        {inApp ? (
          ...
          <div className="flex flex-col gap-3">
            {/* Google (GIS) — never says “Supabase” */}
            {!gisTimeout && <div id="googleLoginBtnMount" />}

            {/* GIS blocked by CSP/ad-block → provide info and rely on email below */}
            {gisTimeout && (
              <button
                onClick={() => alert("Google login couldn’t load here. Try Safari/Chrome or email below.")}
