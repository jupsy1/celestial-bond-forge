...
export function ZodiacSelector({ selectedSign, onSignSelect, className = "" }: ZodiacSelectorProps) {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 cosmic-card border border-border/50 max-h-[480px] overflow-y-auto z-50 bg-background">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3">
            {zodiacSigns.map((zodiac) => (
              <button
                ...
                className="flex items-center space-x-3 p-4 rounded-lg hover:bg-muted/50 transition-colors text-left min-h-[70px]"
              >
